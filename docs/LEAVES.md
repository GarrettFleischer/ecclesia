...
Words are weighed in the skin (`JudgeHub`) and arrive as `Posture`. The first
submit is `VoicePass::Review`; the second is `Publish`. The leaf calls
`require_uplifting`. It does not talk to Jev or OpenRouter. See
[VOICE.md](VOICE.md).
