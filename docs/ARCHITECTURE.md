...
The full contract is in [LEAVES.md](LEAVES.md).
...
`Db::apply` takes `&Effect` and commits writes and notices in one SQLite
...
`.cloned().collect()`, `.to_vec()`, or `Need::from_card`. `us_leaf_01`
fails the build if a leaf file grows I/O.
...
| `leaf/gifts.rs` | endorse any skill, accept/decline, name a gift, profile |
...
`src/sdk` maps machine externals onto those values. Persistence and HTTP are
siblings of `sdk`, not inside it.
...
the need's steward and the person who wrote the offer read the text.
