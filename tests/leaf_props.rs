//! Random inputs against the leaves. A leaf must return Ok or DomainError.
//! It must not panic, and a successful write must look like the person typed.

use ecclesia::leaf::{
    Application, ApplicationStatus, CatalogPresence, Church, DemoSeat, DomainError,
    ...
    invite_code_for, invite_member, may_impersonate, normalize_email, pair_memberships,
...
use ecclesia::sdk::judge::word_gate;
use ecclesia::sdk::session::Session;
...
proptest! {
    ...
    fn us_prop_01_register_is_total(
        ...
        let result = register(
            &name, &email, &city, &region, &bio, availability, posture, "u1".into(), "t1".into(),
        );
    ...
    #[test]
    fn us_prop_02_demo_seat_is_exhaustive(open in any::<bool>()) {
        let seat = if open { DemoSeat::Open } else { DemoSeat::Sealed };
        let result = may_impersonate(seat);
        match seat {
            DemoSeat::Open => assert_eq!(result, Ok(())),
            DemoSeat::Sealed => assert_eq!(result, Err(DomainError::DemoDisabled)),
        }
    }
    ...
    fn us_prop_07_declined_cards_stay_with_the_pair(viewer in "[a-z]{1,8}") {
        let card = ecclesia::leaf::EndorsementCard {
            id: "e1".into(),
            from_user_id: "james".into(),
    ...
    fn us_prop_10_visible_cards_obey_the_rule(
        ...
        let open = ecclesia::leaf::NeedCard {
            id: "n1".into(),
    ...
    fn us_prop_14_session_round_trips(
        ...
        let session = Session::signed_in(user.clone(), csrf);
            ...
            _ => {
                if !matches!(
                    ...
                    "email" | "auth" | "not_found" | "forbidden" | "steward" | "not_yours"
                        ...
                        | "invite" | "pending" | "bad_email" | "tone" | "demo" | "rate"
                ) {
