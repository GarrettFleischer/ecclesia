...
mod tests {
    ...
    #[test]
    fn us_domain_01_domain_does_not_do_io() {
        let mut offenders = Vec::new();
        each_src_line(|path, index, line| {
            if domain_does_io(path, line) {
                offenders.push(format!("{}:{}: {}", path.display(), index + 1, line.trim()));
            }
        });
        assert!(
            offenders.is_empty(),
            "domain takes values and returns Effect; I/O lives in the SDK:\n{}",
            offenders.join("\n")
        );
    }
    ...
    fn domain_does_io(path: &Path, line: &str) -> bool {
