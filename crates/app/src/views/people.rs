...
pub fn me(
    ...
    page(
        ...
        html! {
            ...
            section class="panel" {
                h2 { "Password" }
                form class="stack" method="post" action="/session/password" {
                    (csrf_input(csrf))
            ...
            section class="panel" {
                h2 { "Devices" }
                (device_list(devices, current_session, csrf))
            }
            form class="account-end" method="post" action="/session/logout" {
                (csrf_input(csrf))
                button class="btn btn-quiet" type="submit" { "Sign out" }
            }
            form class="account-end" method="post" action="/session/logout-all" {
                (csrf_input(csrf))
                button class="btn btn-quiet" type="submit" { "Sign out everywhere" }
            }
        },
    )
}

fn device_list(devices: &[SessionRow], current_session: Option<&str>, csrf: &str) -> Markup {
    if devices.is_empty() {
        return html! { p class="muted" { "No devices." } };
    }
    html! {
        div class="stack" {
