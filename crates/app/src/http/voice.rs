...
pub async fn refine_words(
    ...
    if let RateDecision::Refuse = state.decide_rate(RateKind::Refine, &who.0).await {
    ...
    let text = match state.sdk.refine.rewrite(kind, &source).await {
        Ok(text) => text,
        Err(error) => {
    ...
    let seat = if state.sdk.refine.is_live() {
        "live"
    } else {
        "echo"
    };
